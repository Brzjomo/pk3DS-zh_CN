// Auto-generated from makerom pki/test.h
// CA = RSA-4096 signature + RSA-2048 key (0x400)
// XS = RSA-2048 signature + RSA-2048 key (0x300)
// CP = RSA-2048 signature + RSA-2048 key (0x300)
// Total cert chain: 0xA00 bytes
using System;
using System.Security.Cryptography;
using System.Numerics;

namespace pk3DS.Core.CTR;

internal static class DebugPKI
{
internal static readonly byte[] CaCert =
{
    
};

internal static readonly byte[] XsCert =
{
    
};

internal static readonly byte[] CpCert =
{
    
};

internal static readonly byte[] TestModulus =
{
    
};

internal static readonly byte[] TestPrivExponent =
{
    
};

    // RSA-2048 public exponent: 65537
    private static readonly byte[] PublicExponent = { 0x01, 0x00, 0x01 };

    // SHA-256 DigestInfo prefix (DER-encoded): 19 bytes
    private static readonly byte[] SHA256DigestInfoPrefix =
    {
        0x30, 0x31, 0x30, 0x0D, 0x06, 0x09, 0x60, 0x86,
        0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x01, 0x05,
        0x00, 0x04, 0x20
    };

    /// <summary>
    /// Signs data with RSA-2048 PKCS#1 v1.5 SHA-256 using makerom's test key.
    /// Returns 0x100-byte big-endian signature.
    /// </summary>
    internal static byte[] SignRsa2048Sha256(byte[] data)
    {
        return SignRsa2048Sha256(data, 0, data.Length);
    }

    internal static byte[] SignRsa2048Sha256(byte[] data, int dataOffset, int length)
    {
        // 1. SHA-256 hash
        byte[] hash = SHA256.HashData(
            new ReadOnlySpan<byte>(data, dataOffset, length));

        // 2. Build PKCS#1 v1.5 block
        int modLen = TestModulus.Length; // 0x100 = 256
        byte[] block = new byte[modLen];
        block[0] = 0x00;
        block[1] = 0x01;
        int padLen = modLen - 3 - SHA256DigestInfoPrefix.Length - hash.Length;
        for (int i = 0; i < padLen; i++)
            block[2 + i] = 0xFF;
        block[2 + padLen] = 0x00;
        Array.Copy(SHA256DigestInfoPrefix, 0, block, 2 + padLen + 1, SHA256DigestInfoPrefix.Length);
        Array.Copy(hash, 0, block, 2 + padLen + 1 + SHA256DigestInfoPrefix.Length, hash.Length);

        // 3. RSA sign: signature = block^d mod n
        var modulus = new BigInteger(TestModulus, isUnsigned: true, isBigEndian: true);
        var privExp = new BigInteger(TestPrivExponent, isUnsigned: true, isBigEndian: true);
        var message = new BigInteger(block, isUnsigned: true, isBigEndian: true);
        var signature = BigInteger.ModPow(message, privExp, modulus);

        // 4. Output as 256-byte big-endian, right-aligned
        byte[] result = new byte[modLen];
        Span<byte> sigSpan = new byte[modLen];
        if (!signature.TryWriteBytes(sigSpan, out int bytesWritten, isUnsigned: true, isBigEndian: true))
            throw new InvalidOperationException("Failed to write signature bytes");
        // TryWriteBytes writes BE bytes from index 0; right-align into result
        int resultOffset = modLen - bytesWritten;
        sigSpan.Slice(0, bytesWritten).CopyTo(result.AsSpan(resultOffset));
        return result;
    }
}
