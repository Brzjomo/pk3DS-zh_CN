    ...
    public partial class MaisonEditor7 : Form
    {
        ...
        private void Setup()
        {
            ...
            for (int i = 0; i < trNames.Length; i++)
                CB_Trainer.Items.Add($"{trNames[i] ?? "-- 未知 --"} - {i:000}");
            ...
            finalEvoSet = new HashSet<int>(Legal.FinalEvolutions_7);
        }
        ...
        {
            Bitmap rawImg = WinFormsUtil.GetSprite(CB_Species.SelectedIndex, (int)NUD_Form.Value, 0, CB_Item.SelectedIndex, Main.Config);
        ...
        private void B_Randomize_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < pkFiles.Length; i++)
            {
                var pkm = new Maison7.Pokemon(pkFiles[i]);
            ...
            if (CHK_Species.Checked)
            {
                ...
                {
                    var tr = new Maison7.Trainer(trFiles[t]);
                    if (tr.Count > 0)
                    {
        ...
        {
            // Build candidate list (non-Mega, optionally excluding legendaries)
            ...
            // If allowing legendaries, split into two pools
            if (allowLegendary)
            {
        ...
        {
            // Weight by BST^2 for strong bias toward high-BST Pokemon
        ...
        {
            var pi = Main.Config.Personal.Table[species];
            // Stat order for nature: 0=ATK, 1=DEF, 2=SPE, 3=SPA, 4=SPD
            ...
            if (bestIdx == worstIdx)
                return (byte)rand.Next(0, 25); // All equal, fully random

            // Pick nature that boosts best and reduces worst
            // nature = 5 * inc_stat + dec_stat
            int nature = 5 * bestIdx + worstIdx;
            return (byte)nature;
        }

        private static void SetSmartEVs(Maison7.Pokemon pkm, Random rand)
        {
            var pi = Main.Config.Personal.Table[pkm.Species];
            // Stat order for EVs: 0=HP, 1=ATK, 2=DEF, 3=SPE, 4=SPA, 5=SPD
            int[] stats = { pi.HP, pi.ATK, pi.DEF, pi.SPE, pi.SPA, pi.SPD };

            // Get top 3 stat indices by base value
            var ranked = stats.Select((v, i) => (value: v, index: i))
                .OrderByDescending(x => x.value)
                .ToList();

            // Always enable EV for best stat
